...
func PrintUsage(version string) {
	fmt.Printf("进程跟踪器 v%s - Process Tracker\n\n", version)
	fmt.Println("用法:")
	...
	fmt.Println("  today                     显示今日统计")
	fmt.Println("  week                      显示本周统计")
	fmt.Println("  month                     显示本月统计")
	fmt.Println("  details                   显示详细统计")
	fmt.Println("  export                    导出数据为JSON")
	fmt.Println("  cleanup                   清理过期数据")
	fmt.Println("  start-process <名称> [命令]  启动指定进程")
	fmt.Println("  stop-process <名称或PID>    停止指定进程")
	fmt.Println("  help                      显示此帮助信息")
	fmt.Println("")
	fmt.Println("选项:")
	fmt.Println("  --config <路径>           配置文件路径")
	fmt.Println("  --data-file <路径>        数据文件路径")
	fmt.Println("  --interval <秒数>         监控间隔(秒)")
	fmt.Println("")
	fmt.Println("示例:")
	fmt.Println("  process-tracker start")
	fmt.Println("  process-tracker today --config /path/to/config.yaml")
	fmt.Println("  process-tracker start-process chrome google-chrome")
	fmt.Println("  process-tracker stop-process chrome")
	fmt.Println("")
	fmt.Println("配置文件:")
	fmt.Println("  默认配置文件: ~/.process-tracker.yaml")
	fmt.Println("  默认数据文件: ~/.process-tracker.log")
	fmt.Println("")
	fmt.Println("特性:")
	fmt.Println("  🔄 实时监控CPU、内存、磁盘I/O、网络使用")
	fmt.Println("  📊 智能统计: 简单/详细/完整三种粒度")
	fmt.Println("  🗂️  智能分类: 自动识别应用类型")
	fmt.Println("  💾 存储优化: 自动文件轮转和压缩")
	fmt.Println("  🎛️  灵活配置: YAML配置文件支持")
	fmt.Println("  📤 数据导出: JSON格式数据导出")
}
